...
function Diagram({ file, title }: { file: string; title: string }) {
  return <figure className="xd-diagram">
    <PortfolioImage variant="1440" src={asset('portfolio', file)} alt={title} width="100%" loading="lazy" preview={{ mask: '放大查看' }} />
    ...
    <main>
      <div className="hh-container hh-intro" id="home">
        ...
        <div className="hh-cover">
          ...
          <div className="hh-cover-phones">
            <img decoding="async" src={thumbnail(asset('xiaodongya', 'robot-onsite.png'))} alt="杭州东站内的小东鸭机器人实拍" width="720" height="960" fetchPriority="high" />
            <img decoding="async" src={thumbnail(asset('hangxiaodong', 'robot-map.png'))} alt="小东鸭位置、可用状态与召唤界面" width="1206" height="2622" />
          </div>
      ...
      <article className="hh-article">
        ...
        <Section id="overview" title="让旅客问完路，也有人带着走">
          ...
          <figure className="xd-photo"><PortfolioImage src={asset('xiaodongya', 'station-service.jpg')} alt="杭州东站杭小东服务展示区前的小东鸭机器人" width="100%" loading="lazy" preview={{ mask: '查看现场原图' }} /><figcaption>杭州东站现场：杭小东服务入口与小东鸭机器人。</figcaption></figure>
          ...
          ].map(([file,title,copy]) => <figure key={file}><div className="hh-screen"><PortfolioImage variant="480" src={asset('hangxiaodong',file)} alt={title} width="100%" loading="lazy" preview={{mask:'查看原图'}} /></div><figcaption><h3>{title}</h3><p>{copy}</p></figcaption></figure>)}</div></Image.PreviewGroup>
        ...
        <Section id="technology" title="说出目的地，衔接带路与行李辅助">
          ...
          ].map(([file,title,copy]) => <figure className="xd-photo" key={file}><PortfolioImage src={asset('xiaodongya',file)} alt={title} width="100%" loading="lazy" preview={{mask:'查看对话原图'}} /><figcaption><h3>{title}</h3><p>{copy}</p></figcaption></figure>)}</div></Image.PreviewGroup>
        ...
        <Section id="coverage" title="从正式上线，到被看见的站内服务">
          ...
          ].map(([file,title,copy]) => <figure key={file}><PortfolioImage src={asset('xiaodongya',file)} alt={`${title}报道截图`} width="100%" loading="lazy" preview={{mask:'查看报道截图'}} /><figcaption><h3>{title}</h3><p>{copy}</p></figcaption></figure>)}</div></Image.PreviewGroup>
        </Section>
